...
interface NewsData {
  ...
  worldUpdates: NewsArticle[];
  europe: NewsArticle[];
  asia: NewsArticle[];
  africa: NewsArticle[];
  americas: NewsArticle[];
  // Duplicate sections
  worldNews2: NewsArticle[];
  worldUpdates2: NewsArticle[];
  europe2: NewsArticle[];
  asia2: NewsArticle[];
  africa2: NewsArticle[];
  americas2: NewsArticle[];
  worldNews3: NewsArticle[];
  worldUpdates3: NewsArticle[];
  europe3: NewsArticle[];
  asia3: NewsArticle[];
  africa3: NewsArticle[];
  americas3: NewsArticle[];
}
...
const World: React.FC = () => {
  ...
  const loadMoreArticles = async () => {
    ...
    setTimeout(() => {
      const newSection = [
        mockArticle('Breaking: Global summit addresses climate change'),
        mockArticle('International trade agreements reshape economy'),
        mockArticle('World leaders meet for historic peace talks'),
        mockArticle('Global health initiative launches worldwide')
      ];
      ...
      setTimeout(() => {
        setNewsData({
          ...
          europe: [
            ...
            mockArticle('Emerging europe show resilience despite global uncertainties', '/ttttttt.jpg')
          ],
          asia: [
          ...
          europe2: [
            ...
            mockArticle('Real estate europe show strong fundamentals in urban areas', '/wm01.jpeg')
          ],
          asia2: [
          ...
          europe3: [
            mockArticle('Currency europe adapt to changing monetary policy landscapes', '/wm01.jpeg'),
      ...
      <div className="home__container">
        ...
        <section id="categories-section" className="categories-section">
          <div className="categories-grid">
            {/* Markets Column */}
            <div id="europe-column" className="category-column">
              <h2 id="europe-header" className="category-header">
                <a href="/europe" className="category-link">Markets</a>
                <span className="arrow-symbol">›</span>
              </h2>
              
              {newsData.europe.map((article: NewsArticle, index: number) => (
                <article key={index} id={`europe-card-${index + 1}`} className={`category-card ${index === 0 ? 'featured' : ''}`}>
                  {index === 0 && (
            ...
            {/* Finance Column */}
            <div id="asia-column" className="category-column">
              <h2 id="asia-header" className="category-header">
                <a href="/asia" className="category-link">Finance</a>
                <span className="arrow-symbol">›</span>
              </h2>
              
              {newsData.asia.map((article: NewsArticle, index: number) => (
                <article 
                  key={index} 
                  id={`asia-card-${index + 1}`} 
            ...
            {/* Startups Column */}
            <div id="africa-column" className="category-column">
              <h2 id="africa-header" className="category-header">
                <a href="/africa" className="category-link">Startups</a>
                <span className="arrow-symbol">›</span>
              </h2>
              
              {newsData.africa.map((article: NewsArticle, index: number) => (
                <article 
                  key={index} 
                  id={`africa-card-${index + 1}`} 
                  className={`category-card ${index === 0 ? 'featured' : ''}`}
                >
                  {index === 0 && (
                    <div className="category-card-image">
            ...
            {/* Economy Column */}
            <div id="americas-column" className="category-column">
              <h2 id="americas-header" className="category-header">
                <a href="/americas" className="category-link">Economy</a>
                <span className="arrow-symbol">›</span>
              </h2>
              
              {newsData.americas.map((article: NewsArticle, index: number) => (
                <article 
                  key={index} 
                  id={`americas-card-${index + 1}`} 
                  className={`category-card ${index === 0 ? 'featured' : ''}`}
                >
                  {index === 0 && (
                    <div className="category-card-image">
