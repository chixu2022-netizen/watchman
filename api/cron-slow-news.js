/**
 * CRON JOB: Slow News (Slow Updates)
 * 
 * Updates slower categories every 2 hours
 * Categories: health, politics, local
 * 
...
async function fetchNewsData(category, limit) {
  const url = `https://newsdata.io/api/1/news?apikey=${NEWSDATA_API_KEY}&category=${category}&size=${limit}&language=en`;
...
async function fetchNewsAPI(category, limit) {
  ...
  const categoryMap = { local: 'general' };
...
async function fetchGNews(category, limit) {
  ...
  const categoryMap = { local: 'nation', politics: 'nation' };
  const mapped = categoryMap[category] || category;
...
async function fetchMediaStack(category, limit) {
  ...
  const url = `https://api.mediastack.com/v1/news?access_key=${MEDIASTACK_KEY}&categories=${category}&languages=en&limit=${limit}`;
...
/**
 * Cleanup old articles (run once during this job)
 */
async function cleanupOldArticles() {
  try {
    ...
    if (error) throw error;
    console.log('🧹 Cleaned up old articles (30+ days)');
  } catch (error) {
...
/**
 * Main cron handler - SLOW NEWS (Every 2 hours)
 */
export default async function handler(req, res) {
  const startTime = Date.now();
  console.log('🐢 SLOW NEWS CRON STARTED:', new Date().toISOString());
  
  try {
    // Slow-moving categories (updated every 2 hours)
    const slowCategories = ['health', 'politics', 'local'];
    
    let totalArticles = 0;
    const results = [];
    
    for (const category of slowCategories) {
      try {
        console.log(`📰 Fetching ${category}...`);
        const articles = await fetchWithFallback(category, 10);
    ...
    // Cleanup old articles once per run
    await cleanupOldArticles();
    
    const duration = ((Date.now() - startTime) / 1000).toFixed(2);
    
    console.log(`✅ SLOW NEWS CRON COMPLETED in ${duration}s`);
    ...
    console.error('❌ SLOW NEWS CRON FAILED:', error);
